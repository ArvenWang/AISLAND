...
- 真实 API 验证（deepseek-v4-flash）：seed 101 从 3 角色 5 天内全渴死且
  ...
  （生存机制合理行为）。
- 遗留：部分角色探索推进仍慢（每决策 2-5 格）；move no_path 拒绝仍多
  （知识隔离正常反馈）；社交事件仍 0；P6 UI / P7 verify / P8 验收未开始。
...
- 地图场景中的树/岩石/箱子/残骸/物品/火焰全部替换为真实素材，接入 MapScene。
- P5 对话与社会（独立发言、呼喊、传闻、说谎）、P6 UI 与视觉动作。
- 继续迭代：真实 API 生存率（找水闭环）、社交涌现、P6 UI、P7 verify:mvp2、
  P8 全量验收（A≥6/B≥3/C3/D≥6 对 + 截图包 + 因果链）。
